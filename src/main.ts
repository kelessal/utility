...
class Utility implements IUtility {
    public readonly is: Is = new Is(this)
    public readonly as: As = new As(this)
    public readonly tree: Tree = new Tree(this)
    ...
    public getId(item: any): string | void {
    ...
    public findById(list: any, id: string) {
      id = this.as.id(id)
    ...
    public replaceBy(list: any, newItem: any, oldItem?: any) {
      list = this.as.array(list)
    ...
    public indexOf(list: any, item: any): number {
    ...
    public any(items: any, filter?: Function) {
    ...
    public changeIndex(arr: any, oldindex: number, newindex: number) {
    ...
    public includes(list: any, item: any) {
      return this.indexOf(list, item) >= 0
    }
    public distinct(items: any) {
      var map = new Map()
    ...
    public last(items: any) {
      items = this.as.array(items)
      return items.length === 0 ? undefined : items[items.length - 1]
    }
    public groupBy(items: any, prop: string | Function): { _id: string; children: [] }[] {
      items = this.as.array(items)
      var map = new Map()
      items.forEach((p: any) => {
        var key = this.is.function(prop) ? (prop as Function).call(this, p) : this.get(p, prop as string)
        if (!map.has(key)) {
          map.set(key, [])
        }
        map.get(key).push(p)
    ...
    public take(items: any, length: number): any[] {
      const result = [] as any[]
    ...
    public skip(items: any, length: number) {
      const result = [] as any[]
      length = this.as.integer(length)
      items = this.as.array(items)
    ...
    public sort(items: any, property: string | Function, descending: boolean = false) {
      items = this.as.array(items)
    ...
    public sortNumeric(items: any, property: string | Function, descending: boolean = false) {
      items = this.as.array(items)
      items.sort((a: any, b: any) => {
        if (this.is.function(property)) {
          a = (property as Function)(a)
    ...
    public emptyFn() {}
    public async emptyAsyncFn() {}
    public echoFn(): (val: any) => any {
      return function (val) {
        return val
      }
    }
    public trueFn(): Function {
      return function () {
        return true
      }
    }
    public random(min: number, max: number, digit: number): number {
    ...
    public equals(a: any, b: any): boolean {
      ...
      if (a && b && typeof a == 'object' && typeof b == 'object') {
        ...
        if (arrA && arrB) {
          ...
          for (i = length; i-- !== 0; ) if (!this.equals(a[i], b[i])) return false
          return true
        }
    ...
    public hasAllFields(value: { [key: string]: any }, fields: string[]): boolean {
    ...
    public extend(out: any, ...args: any[]) {
      out = out || {}
    ...
    public hasProperty(obj: { [key: string]: any }, path: string): boolean {
    ...
    public set(obj: { [key: string]: any }, path: string, value: any) {
      var paths = path.split('.')
      var current = obj
    ...
    public seperate(str: string, seperator: string): string[] {
      if (!str) {
        return [str, '']
      }
    ...
    public trimBefore(str: string, trimSep: string): string {
      if (!str) return ''
      let index = str.indexOf(trimSep)
      return index < 0 ? str : str.substring(index + trimSep.length)
    }
    public trimThen(str: string, trimSep: string): string {
      if (!str) return ''
      let index = str.indexOf(trimSep)
      return index < 0 ? str : str.substring(0, index)
    }
    public trimLeft(str: string, trimSep: string): string {
      if (!str) return str
      return str.startsWith(trimSep) ? str.substring(trimSep.length) : str
    }
    public trimRight(str: string, trimSep: string): string {
      if (!str) return str
    ...
    public jwtDecode(token: string) {
      ...
      let parsedToken = JSON.parse(Buffer.from(base64, 'base64').toString('binary'))
    ...
    public waitAsync(timeout: number): Promise<void> {
      return new Promise((resolve) => {
        setTimeout(resolve, timeout)
      })
    }
    public debounce(fn: Function, timeout: number) {
      return new Debounce(fn, timeout)
    }
    public throttle(fn: Function, timeout: number) {
      return new Throttle(fn, timeout)
    }
    public combineUrl(base: string, relative: string) {
